 ...
 * Copyright (c) 2017, Zhuravlev Yury aka stalkerg
 */

import mxClient from '../mx_client';
import mxEvent from './mx_event';
import mxPoint from './mx_point';
import mxObjectIdentity from './mx_object_identity';
import mxConstants from './mx_constants';

const closeResource = (mxClient.language != 'none') ? 'close' : '';

const isVml = node => node != null && node.tagUrn == 'urn:schemas-microsoft-com:vml';

const mx_isNaN = value => typeof(value) === 'number' && isNaN(value);

const toRadians = deg => Math.PI * deg / 180;

const contains = (bounds, x, y) => (bounds.x <= x && bounds.x + bounds.width >= x &&
																		bounds.y <= y && bounds.y + bounds.height >= y);


const errorImage = mxClient.imageBasePath + '/error.gif';

const errorResource = (mxClient.language != 'none') ? 'error' : '';
...
const indexOf = (array, obj) => Array.prototype.indexOf.call(array, obj);

function indexOfStylename(style, stylename) {
...
function removeCursors(element) {
...
function show(graph, doc, x0, y0, w, h) {
	x0 = (x0 != null) ? x0 : 0;
	y0 = (y0 != null) ? y0 : 0;
...
function link(parent, text, funct, pad) {
...
function getDocumentScrollOrigin(doc) {
...
function getScrollOrigin(node, includeAncestors, includeDocument) {
	...
	while (node != null && node != b && node != d)
	{
		...
		var style = getCurrentStyle(node);
	...
	{
		var origin = getDocumentScrollOrigin(doc);
...
function getOffset(container, scrollOffset) {
	var offsetLeft = 0;
	var offsetTop = 0;
	...
	{
		var offset = getDocumentScrollOrigin(container.ownerDocument);
		offsetLeft += offset.x;
		offsetTop += offset.y;
	}
...
function setStyleFlag(style, key, flag, value) {
	if (style == null || style.length == 0)
	{
		if (value || value == null)
		{
...
function get(url, onload, onerror, binary, timeout, ontimeout) {
...
function getRotatedPoint(pt, cos, sin, c) {
...
export default {
	...
	{
		return link(parent, text, function()
		{
	...
	fit: function(node)
	{
		...
		var offset = getDocumentScrollOrigin(node.ownerDocument);
		var sl = offset.x;
		var st = offset.y;
	...
	equalEntries: function(a, b)
	{
		...
		else if (a != null && b != null)
		{
			...
			for (var key in a)
			{
				...
				if ((!mx_isNaN(a[key]) || !mx_isNaN(b[key])) && a[key] != b[key])
				{
					return false;
				}
			}
	...
	toString: function(obj)
	{
	    ...
	    for (var i in obj)
	    {
	    	try
	    	{
		        ...
		        {
		        	var ctor = getFunctionName(obj[i].constructor); 
		            output += i + ' => [' + ctor + ']\n';
		        }
		        else
		        {
	...
	{
		var value = getValue(terminal.style, mxConstants.STYLE_PORT_CONSTRAINT,
			getValue(edge.style, (source) ? mxConstants.STYLE_SOURCE_PORT_CONSTRAINT :
		...
		else
		{
			...
			var constraintRotationEnabled = getValue(terminal.style, mxConstants.STYLE_PORT_CONSTRAINT_ROTATION, 0);
			var rotation = 0;
			
			if (constraintRotationEnabled == 1)
			{
				rotation = getValue(terminal.style, mxConstants.STYLE_ROTATION, 0);
			}
	...
	findNearestSegment: function(state, x, y)
	{
		...
		if (state.absolutePoints.length > 0)
		{
			...
			for (var i = 1; i < state.absolutePoints.length; i++)
			{
				...
				var dist = ptSegDistSq(last.x, last.y,
	...
	{
		var d = getValue(style, mxConstants.STYLE_DIRECTION, mxConstants.DIRECTION_EAST);
		flipH = (flipH != null) ? flipH : getValue(style, mxConstants.STYLE_FLIPH, false);
		flipV = (flipV != null) ? flipV : getValue(style, mxConstants.STYLE_FLIPV, false);
	...
	getPerimeterPoint: function (pts, center, point)
	{
		...
		{
			var pt = intersection(pts[i].x, pts[i].y, pts[i + 1].x, pts[i + 1].y,
	...
	intersectsHotspot: function(state, x, y, hotspot, min, max)
	{
		...
		if (hotspot > 0)
		{
			...
			var start = getValue(state.style, mxConstants.STYLE_STARTSIZE) * state.view.scale;

			if (start > 0)
			{
				if (getValue(state.style, mxConstants.STYLE_HORIZONTAL, true))
				{
					cy = state.y + start / 2;
					h = start;
				}
			...
			var alpha = toRadians(getValue(state.style, mxConstants.STYLE_ROTATION) || 0);
			...
			if (alpha != 0)
			{
				...
				var pt = getRotatedPoint(new mxPoint(x, y), cos, sin, cx);
				x = pt.x;
				y = pt.y;
			}
		...
		if (cells != null && cells.length > 0)
		{
			...
			try
			{
				for (var i = 0; i < cells.length; i++)
				{
					if (cells[i] != null)
					{
						var style = setStyle(model.getStyle(cells[i]), key, value);
	...
	error: function(message, width, close, icon)
	{
		...
		img.setAttribute('src', icon || errorImage);
		...
		var warn = new mxWindow(mxResources.get(errorResource) ||
			errorResource, div, (w-width)/2, h/4, width, null,
			false, true);
		...
		if (close)
		{
			...
			write(button, mxResources.get(closeResource) ||
				closeResource);
